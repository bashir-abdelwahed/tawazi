    ...
    *,
    ...
    while len(graph):
        ...
        if (
            ...
            logger.debug("Waiting for ExecNodes %s to finish. Finished running %s", running, done)
        ...
        for id_, fut in futures.items():
            if fut.done() and id_ in graph:
                logger.debug("Remove ExecNode %s from the graph", id_)
        ...
        if xn.resource == Resource.thread:
            ...
            logger.debug("Remove ExecNode %s from the graph", xn.id)
...
def handle_exception(
    ...
    if behavior == ErrorStrategy.permissive:
        ...
        raise NotImplementedError("Unknown behavior name: %s", behavior) from e
